...
This repository provides the core logic and data interactions for the **`Users`** domain within our application. It acts as an abstraction layer, handling all necessary communication with our backend services and the **RDS database** to manage `Users`-related data.
...
* **RDS Database Interaction:** Leverages our custom **RDS SDK** to fetch and manage data related to `Users`.
...
credentials_repository/
  ...
  │    │    ├── credentials_repository.dart  # Main repository logic and interface
  │    │    └── failures.dart              # Custom failure definitions for this domain
  │    │
  │    └── credentials_repository.dart  # Exports the repository as a consumable library
  │
...
1.  Add it to the main `pubspec.yaml`:
    ...
    dependencies:
        credentials_repository:
            path: packages/credentials_repository
    ```
2.  Run `flutter pub get`
3.  Inject the `CredentialsRepository()` into Cubits or other service layers.
