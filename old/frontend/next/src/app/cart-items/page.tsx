...
import { findCartItems, deleteCartItems } from "@/actions/cart-items";
...
export default function CartItemsPage() {
  ...
  useEffect(() => {
    const fetchCartItems = async () => {
      setLoading(true);
      const res = await findCartItems({ limit: 10 });
  ...
  const removeCartItems = async (id: string | null) => {
    if (!id) return;
    setLoading(true);
  ...
  return (
    <main className="p-8 max-w-7xl mx-auto">
      <h1 className="text-4xl font-bold mb-6">Table: CartItems</h1>
      ...
      <div className="mb-8 overflow-x-auto rounded-md border border-base-300 shadow-lg bg-base-100">
        <table className="w-full border-collapse">
          ...
          <tbody>
            ...
            <tr className="border-t border-border">
              <td className="text-primary p-3">cart_id</td>
              <td className="text-secondary p-3">
                <span className="bg-primary-light/20 px-2 py-1 rounded text-primary">
                  string
                </span>
            ...
            <tr className="border-t border-border">
              <td className="text-primary p-3">unit_price</td>
              <td className="text-secondary p-3">
                <span className="bg-primary-light/20 px-2 py-1 rounded text-primary">
                  number
                </span>
