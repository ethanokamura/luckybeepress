...
class CreateCartItemsPage extends AppStatefulWidget {
  const CreateCartItemsPage({super.key});

  @override
  State<CreateCartItemsPage> createState() => _CreateCartItemsPageState();
}

class _CreateCartItemsPageState extends State<CreateCartItemsPage> {
        ...
        child: Column(
          children: [
            AppCard(
              child: AppColumn(
                children: [
                  AppHeading.h3('CartItems Values'),
                  ...List.generate(
                    nonNull.length,
                    (index) => AppTextField(
                      semanticLabel:
                  ...
                  AppSpacer(),
                  AppButton(
                      semanticLabel: 'create-cart-items',
                      text: 'Create',
                      onPressed: () async {
                        try {
                          await BlocProvider.of<CartItemsCubit>(context)
                              .createCartItems(
                            data: _generateJson(),
                          );
                          ...
                          AppSnackbar.showError(
                            context: context,
                            message: 'Failed to create a cart items',
                          );
                        }
                      }),
                ],
